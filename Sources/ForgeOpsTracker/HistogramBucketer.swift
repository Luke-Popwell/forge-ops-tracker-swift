...
///
/// `boundariesMs` is duplicated on the server side, in `app/services/histogram_percentile.rb`.
...
enum HistogramBucketer {
    static let boundariesMs: [Double] = [50, 100, 250, 500, 1000, 2500, 5000, 10_000]

    /// Returns the label of the smallest boundary `durationMs` fits under, or `"inf"` for anything
    /// larger than the largest boundary. A String, not a number: this travels as a JSON object key
    /// once flushed, and JSON object keys are always strings.
    static func bucketFor(_ durationMs: Double) -> String {
        guard let boundary = boundariesMs.first(where: { durationMs <= $0 }) else { return "inf" }
        return String(Int(boundary))
    }
}
